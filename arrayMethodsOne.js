...
// const evenNumbers = mixedNumbers.filter(element => element % 2 === 0) // Arrow Method
...
// CODE HERE
const postTaxPrices = prices.map(function(numberPrices) {
  return numberPrices * 1.07
});
...
// CODE HERE
const totalPopulation = populations.reduce(function (accumulatorNumber, populationNumbers) {
  return accumulatorNumber + populationNumbers
})
...
const pokemon = monstersInYourPocket.filter((monster) => monster.CP > 200).map(elm => elm) //There is an error

// const pokemon = monstersInYourPocket.monster.filter(function())
...
const totalPricePlusTax = orders.map((ordersElement) => ordersElement.price * (1 + ordersElement.tax))

console.log(totalPricePlusTax)
console.log("----------")
////////// PROBLEM 6 //////////
...
const bobsTotal = purchases.filter((ownerElement) => ownerElement.owner === "Bob").reduce((arrNumb, bobElement) => arrNumb + bobElement.price, 0) // why does this work??????? WHY THE ZERO?!?!!?
